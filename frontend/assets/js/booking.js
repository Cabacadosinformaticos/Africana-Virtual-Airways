...
async function loadSeatMap() {
  ...
  try {
    // Build list of all segments (outbound + return)
    const allSegments = [
      ...bookingData.itinerary.segments,
      ...(bookingData.returnItinerary ? bookingData.returnItinerary.segments : [])
    ];

    // Fetch seat maps for outbound
    ...
    // Fetch seat maps for return if round-trip
    const returnMaps = bookingData.returnItinerary
      ? await Promise.all(
...
function generateSegmentSeatMap(mapData, segmentIdx, legType, cabinClass) {
  const { segment, occupiedSeats } = mapData;
  ...
  html += `<div class="seat-map-header" style="margin-bottom:16px;">
    <div style="display:inline-block;background:var(--red-100);color:var(--red-700);padding:4px 12px;border-radius:100px;margin-bottom:8px;font-size:0.65rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;">
      ${escapeHtml(segment.flightNumber)} · ${escapeHtml(segment.aircraft?.type || 'Aircraft')} · ${escapeHtml(capitalise(cabinClass))} · ${rows} rows
    </div>
  ...
  html += `<div class="seat-map" id="seatMap-${segmentKey}" style="display:grid;grid-template-columns:repeat(7,1fr);gap:6px;margin-bottom:16px;">`;
...
function selectSeat(seatId, segmentKey) {
  ...
  if (label) label.textContent = `Selected: Seat ${seatId}`;
  updateSelectedSeatsLabel();
}
...
function activateStepIndicator(step) {
  /* Purpose: applies the surrounding side effect to each item in the current collection. Connection: feeds the surrounding collection pipeline inside the booking checkout flow on the public site. */ [1, 2, 3].forEach(currentStep => {
