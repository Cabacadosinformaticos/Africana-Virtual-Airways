/**
 * Africana Virtual Airways — Flight Network
 *
 * ROUTES: Explicit flat list of all operated routes with confirmed flight numbers.
 * Each entry is one directional route; outbound and return are separate entries.
 *
 * Fixes applied vs original user list:
 *  - AFV236 duplicate → FVRG→FQMA keeps AFV236, FQMA→FVRG gets AFV235
 *  - AFV203/AFV204 same direction → AFV204 FQNC→FAOR, AFV203 FAOR→FQNC (return)
 *  - AFV465 duplicate → FQMA→YMML keeps AFV465, FQMA→NZAA gets AFV467
 *  - Typo adv404  → AFV404
 *  - Typo sgbr    → SBGR
 *  - Typo fyhw    → FYWH
 */
...
const ROUTES = [
  ...
  // ── Maputo hub (FQMA) — regional Mozambique (FQNC spokes) ───────────────
  ...
  // ── Maputo hub (FQMA) — Middle East & Indian Ocean ───────────────────────
  ...
  // ── Maputo hub (FQMA) — Asia & Middle East ───────────────────────────────
